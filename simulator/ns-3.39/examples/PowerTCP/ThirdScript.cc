...
void monitor_buffer(FILE* qlen_output, NodeContainer *n) {
	for (uint32_t i = 0; i < n->GetN(); i++) {
		if (n->Get(i)->GetNodeType() == 1 ) { // is switch
            ...
            Ptr<TORSwitchNode> torsw = DynamicCast<TORSwitchNode>(n->Get(i));
            if (queue_result.find(i) == queue_result.end())
				queue_result[i];
            for (uint32_t j = 1; j < torsw->GetNDevices(); j++) {
				uint32_t size = 0;
				for (uint32_t k = 0; k < SwitchMmu::qCnt; k++)
					size += torsw->m_mmu->egress_bytes[j][k];
				queue_result[i][j].add(size);
			}
        }
	}
...
int main(int argc, char *argv[])
{
	...
	std::string confFile = "/home/lbb/OpticalSimulation/simulator/ns-3.39/examples/PowerTCP/config-workload.txt";
	std::string cdfFileName = "/home/lbb/OpticalSimulation/simulator/ns-3.39/examples/PowerTCP/websearch.txt";
