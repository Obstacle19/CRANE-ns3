...
void monitor_buffer(FILE* qlen_output, NodeContainer *n) {	//监视所有switch的buffer情况
...
int main(int argc, char *argv[])
{
	...
	std::string confFile = "/home/lbb/PowerTCP/simulator/ns-3.39/examples/PowerTCP/config-burst.txt";
	std::cout << confFile;
	CommandLine cmd;
	...
	// config switch 配置交换机的特性，各种参数，ECN、PFC等等
	...
	//
	// install RDMA driver 	给服务器安装RDMA驱动
	//
	for (uint32_t i = 0; i < node_num; i++) {
		if (n.Get(i)->GetNodeType() == 0) { // is server
			// create RdmaHw
