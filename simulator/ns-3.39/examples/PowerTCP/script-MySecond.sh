source config.sh
configFile=$NS3/examples/PowerTCP/config-second.txt
RES_DUMP=$NS3/examples/PowerTCP/dump_second
...
for algorithm in ${algs[@]};do
	...
	while [[ $(ps aux|grep "my-second-optimized"|wc -l) -gt 38 ]];do
	...
	time ./waf --run "my-second --conf=$configFile --algorithm=${CCMODE[$algorithm]} --wien=$wien --delayWien=$delay --windowCheck=$window" > $RESULT_FILE  2> $RESULT_FILE &
done




while [[ $(ps aux|grep "my-second-optimized"|wc -l) -gt 1 ]];do
	echo "Waiting for cpu cores.... $N-th experiment "
	sleep 5
done
