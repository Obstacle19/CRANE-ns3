...
int main(int argc, char* argv[])
{
	...
	std::string confFile = "/home/lbb/OpticalSimulation/simulator/ns-3.39/examples/PowerTCP/config-second.txt";
	std::cout << confFile;
	CommandLine cmd;
    ...
    while (!conf.eof())
	{
		std::string key;
		conf >> key;
		if (key.compare("ENABLE_QCN") == 0)	//开启QCN
		{
			uint32_t v;
			conf >> v;
			enable_qcn = v;
		...
		else if (key.compare("CLAMP_TARGET_RATE") == 0) //是否限制目标速率
		{
			uint32_t v;
			conf >> v;
			clamp_target_rate = v;
		...
		else if (key.compare("L2_ACK_INTERVAL") == 0)	//数据层ACK发送间隔
		{
			uint32_t v;
			conf >> v;
			l2_ack_interval = v;
	...
	uint32_t node_num, oswitch_num, switch_num, tors, link_num;
    //139 1 10 4 148
    ...
    for (uint32_t i = 0; i < node_num; i++) {
        ...
        else if (node_type[i] == 3){ // optical switch
            Ptr<OSwitchNode> ocs = CreateObject<OSwitchNode>();
            OpticalSwitch = ocs;
            n.Add(ocs);
			allNodes.Add(ocs);
        ...
        else if (node_type[i] == 1){    //tor switch
    ...
    for (uint32_t i = 0; i < link_num; i++)
	{
		...
		NetDeviceContainer d = qbb.Install(snode, dnode);//Helper帮助两个Node构建netdevice，并且连接到channel中
		if (snode->GetNodeType() == 0) {//判断是否为server，是则分配IP地址
			Ptr<Ipv4> ipv4 = snode->GetObject<Ipv4>();
			ipv4->AddInterface(d.Get(0));
			ipv4->AddAddress(1, Ipv4InterfaceAddress(serverAddress[src], Ipv4Mask(0xff000000)));
		}
		if (dnode->GetNodeType() == 0) {	//des为server
		...
		if (!snode->GetNodeType()) {	//src为switch
    ...
    */
    OpticalSwitch->SetAttribute("ReconfigureDelay", UintegerValue(5000000));
    OpticalSwitch->SetAttribute("OswitchRate", UintegerValue(400000000000));
    OpticalSwitch->SetAttribute("PswitchRate", UintegerValue(100000000000));
	OpticalSwitch->SetAttribute("DurationExtentionRate", DoubleValue(1.5));
	...
	//OpticalSwitch->Reconfigure();
	// std::cout << "Reconfigure over.\n";
